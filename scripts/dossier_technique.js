...
const QA = [
  ...
  ["« Votre 5,31 %, c'est avant ou après frais ? »",
   "**Avant.** Les frais d'instruments représentent 47 k€ par an, soit 0,05 % du patrimoine, et ne sont pas déduits du rendement espéré. Le seuil de 4 %, lui, est un objectif **net**. La comparaison honnête ajoute donc au seuil les frais d'instruments, les frais de mandat, et la friction fiscale si on la réintègre — ce qui porte le seuil entre 4,05 % et 4,85 % selon le périmètre, et ramène la marge entre +0,5 et +1,26 point. **C'est une faiblesse connue du dossier, non encore corrigée.**"],
  ...
  ["« Le backtest fait 4,47 % pour un objectif de 4 %. C'est très juste. »",
   "**La comparaison n'est pas légitime en l'état, et elle joue contre nous.** Le seuil de 4 % est bâti pour un monde à 4 % d'inflation ; la période 2006-2026 a connu une inflation d'environ 2 %. Jugé contre l'inflation **réalisée** de la période, le 4,47 % représente environ +2,4 points de rendement réel, ce qui est un net succès et non une réussite de justesse. C'est exactement le piège de régime signalé au § 3.1."],
...
corps.push(H2("9.3 Le seuil net comparé à un rendement brut — non corrigé"));
corps.push(P("L'objectif de 4 % est net de frais ; les rendements espérés sont bruts. Une version antérieure du dossier faisait le calcul complet — 4,00 % d'inflation + 0,55 % de frais + 0,30 % de friction fiscale = 4,85 % de rendement brut requis — et la version actuelle l'a perdu. **La marge annoncée de +1,31 point est donc surévaluée** ; la marge réelle se situe entre +0,5 et +1,26 point selon le périmètre retenu."));

corps.push(H2("9.4 Le backtest jugé dans le mauvais régime d'inflation — non corrigé"));
corps.push(P("Le rendement réalisé de 4,47 % est comparé à un seuil de 4 % bâti pour un monde à 4 % d'inflation, alors que la période a connu environ 2 %. Cette erreur joue **en défaveur** du portefeuille : correctement jugé, le résultat est bien meilleur qu'il n'en a l'air. Le chiffrage exact demande la série d'inflation zone euro sur vingt ans, qui n'est pas encore dans le dossier."));

corps.push(H2("9.5 La fréquence des années sous l'objectif — absente du deck"));
corps.push(P("40,8 % des années glissantes du portefeuille retenu sont sous 4 %. Le chiffre est calculé par l'application mais n'apparaît pas dans la présentation. Mieux vaut le donner soi-même que se le faire sortir."));
